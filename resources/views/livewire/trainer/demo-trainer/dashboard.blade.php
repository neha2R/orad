<section x-data="{lead:leadModal(), history:historyModal(), feedback:feedbackModel(), reschedule:rescheduleModel(), demolink:demoLinkModel() }">
    ...
    <div class="card-section grid gap-4 lg:grid-cols-2 grid-col-1 ">
        ...
        <div class="card-header {{$leadStatus == 0 ? 'shadow-2xl' : 'shadow-md'}}" wire:click="demoStatus(0)">
            <span class="card-title new-lead">
                {{ $pendingDemo ?? '' }}</span>
            <p class="card-subtitle">Pending Demo</p>
        </div>
        ...
        <div  class="card-header {{$leadStatus == 1 ? 'shadow-2xl' : 'shadow-md'}}" wire:click="demoStatus(1)">
            ...
            <p class="card-subtitle">Demo Done</p>
        </div>
    ...
    <div class="table-section ">
            ...
            <div>
                <div class="flex justify-between items-center">
                    ...
                    <div class="">
                        ...
                        <button class="btn-warning" x-on:click="feedback.feedbackOpen()">
                            Feedback
                        </button>
                      
                        <button class="btn-primary" x-on:click="reschedule.rescheduleOpen()">
                            Reschuedule
                        </button>
                    </div>
                </div>
            ...
            <div class=" min-w-full shadow rounded-lg overflow-hidden">
                <table class="min-w-full leading-normal">
                    ...
                    <tr>
                        ...
                        <th class="th" align="left">Demo Slot</th>
                    ...
                    <tbody class="transition duration-700 ease-in-out">
                        @forelse ($data as $key => $item)
                            ...
                            <tr>
                                ...
                                <td class="td">
                                    <p class="text-gray-500">{{ optional($item->userRelation)->mobilecode ?? '' }}{{ optional($item->userRelation)->mobile ?? 'N/A' }}</p>
                                </td>
                                <td class="td">
                                    <p class="p">
                                ...
                                <td class="td">
                                    @if($item->demolink)
                                    <a class="btn-success" target ="_blank" href="{{ $item->demolink }}">
                                        Join Link
                                    </a>
                                    @else
        ...
        {{-- Reschedule model  --}}
        @include('includes.reschedule', ['method'=>'rescheduleDemo'])
