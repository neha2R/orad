<section x-data="{lead:leadModal(), history:historyModal(), followup:followupModal(), payment:paymentModal() }">
    ...
    <div class="card-section grid gap-4 lg:grid-row-2 lg:grid-cols-4 grid-col-1 ">
        ...
        <div class="card-header {{$assignedtable == 1 ? 'shadow-2xl' : 'shadow-md'}}" wire:click="leadstatusstatsactive(1)">
            <span class="card-title">  {{ $newleads ?? '' }}</span>
        ...
        <div class="card-header {{$assignedtable == 3 ? 'shadow-2xl' : 'shadow-md'}}" wire:click="leadstatusstatsactive(3)">
            <span class="card-title">  {{ $paymentlinksend ?? '' }}</span>
            <p class="card-subtitle">Payment Link Sent</p>
        </div>
  
        <div  class="card-header {{$assignedtable == 4 ? 'shadow-2xl' : 'shadow-md'}}" wire:click="leadstatusstatsactive(4)">
            <span class="card-title"> {{ $converted ?? '' }} </span>
        ...
        <div class="py-8" >
            <div>
                <div class="flex justify-between items-center">
                    ...
                    <div class="">
                        <button class="btn-primary" x-on:click="payment.paymentOpen()">
                            Payment Link
                        </button>
                    </div>
                </div>
